import Link from 'next/link';
import Image from 'next/image';

export default function Footer() {
  return (
    <footer className="footer">
      <div className="container">
        <div className="footer-grid">
          {/* Brand Info */}
          <div className="footer-brand">
            <Link href="/" style={{ display: 'inline-block', marginBottom: '12px' }}>
              <Image
                src="/images/logo.png"
                alt="FarOha Brand"
                width={170}
                height={55}
                style={{ objectFit: 'contain', height: '48px', width: 'auto', borderRadius: '6px' }}
              />
            </Link>
            <p>
              FarOha_Brand بتقدملك ملابس للمرأة اللي بتحب الحشمة، الراحة، والأناقة البسيطة.
              تصاميم مريحة وبأجود الأقمشة لتتناسب مع أسلوب حياتك اليومي.
            </p>
            <div className="footer-social">
              {/* WhatsApp */}
              <a
                href="https://wa.me/201006955864"
                target="_blank"
                rel="noreferrer"
                title="واتساب (01006955864)"
              >
                <svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946.003-6.556 5.338-11.891 11.893-11.891 3.181.001 6.167 1.24 8.413 3.488 2.245 2.248 3.481 5.236 3.48 8.414-.003 6.557-5.338 11.892-11.893 11.892-1.99-.001-3.951-.5-5.688-1.448l-6.305 1.654zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884-.001 2.225.651 3.891 1.746 5.634l-.999 3.648 3.742-.981z"/>
                </svg>
              </a>
              {/* Instagram */}
              <a
                href="https://www.instagram.com/faroha_brand_eg?igsh=MXJ0aGQ4NWtnMXFlMA=="
                target="_blank"
                rel="noreferrer"
                title="إنستغرام"
              >
                <svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
                </svg>
              </a>
              {/* Facebook */}
              <a
                href="https://www.facebook.com/share/1DnoR36wAd/"
                target="_blank"
                rel="noreferrer"
                title="فيسبوك"
              >
                <svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                </svg>
              </a>
            </div>
          </div>

          {/* Quick Links */}
          <div>
            <h4 className="footer-heading">روابط سريعة</h4>
            <div className="footer-links">
              <Link href="/">الرئيسية</Link>
              <Link href="/shop">المتجر</Link>
              <Link href="/about">من نحن</Link>
              <Link href="/contact">تواصلي معنا</Link>
            </div>
          </div>

          {/* Categories */}
          <div>
            <h4 className="footer-heading">الأقسام</h4>
            <div className="footer-links">
              <Link href="/shop?category=esdals">إسدالات صلاة</Link>
              <Link href="/shop?category=dresses">دريسات أنيقة</Link>
              <Link href="/shop?category=loose">ملابس واسعة</Link>
              <Link href="/shop?category=offers">عروض خاصة</Link>
            </div>
          </div>

          {/* Customer Service & Policies */}
          <div>
            <h4 className="footer-heading">خدمة العملاء</h4>
            <div className="footer-links">
              <Link href="/return-policy">سياسة الاستبدال والإرجاع</Link>
              <Link href="/cart">سلة التسوق</Link>
              <Link href="/checkout">إتمام الطلب</Link>
              <Link href="/admin/login">دخول الإدارة</Link>
            </div>
          </div>
        </div>

        {/* Footer Bottom */}
        <div className="footer-bottom">
          <p>© {new Date().getFullYear()} FarOha_Brand. جميع الحقوق محفوظة — الأناقة في كل التفاصيل.</p>
        </div>
      </div>
    </footer>
  );
}
